...
<form class="needs-validation col-6 mx-auto" name="signinForm" method="post" action="register.php" onsubmit="checkForm()" validate>
  ...
  <div class="my-3">
    ...
    <input type="password" class="form-control" id="password_check" placeholder="password_check" name = "password_check" oninput="setCustomValidity('');" onchange="if(document.getElementById('password').value != document.getElementById('password_check').value){setCustomValidity('密碼不相同');}" required />
    <div class="valid" id="passwordCheckStatus">
    </div>
  </div>
  <div class="my-3">
    <label for="telephone">手機號碼</label>
    <input type="text" class="form-control" id="telephone" placeholder="telephone" name="telephone" pattern="09\d{2}-?\d{3}-?\d{3}" required>
    <div class="invalid-feedback">
      請輸入正確姓名
    </div>
    <div class="valid-feedback">
